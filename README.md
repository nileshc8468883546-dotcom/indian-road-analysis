# indian-road-analysi## 🚗 Indian Road Accident Analysis Dashboard | 

 ### 📌 Project Overview

 Developed an interactive **Excel dashboard** to analyze Indian road accident data and identify patterns across states, road types, traffic density, accident severity, risk levels, and accident causes. The dashboard covers **20,000 accidents and 34,529 casualties** and uses interactive filters for **State, Risk Category, Time Period, and Road Type** to make the analysis easy to explore.

 ### 🔍 Key Insights

 - **Maharashtra** recorded the highest number of accidents among the states analyzed.
- Accident records were compared across **High, Medium, and Low traffic-density** categories.
- **Urban, rural, and highway** accident patterns were analyzed separately.
- Accident severity was categorized into **Fatal, Major, and Minor**.
- **Distraction and overspeeding** appeared among the leading recorded accident causes.
- The dashboard provides an overall **average risk score of 0.438**.
- The analysis highlights how accident patterns can vary based on **location, road type, traffic density, and cause**.

 ### 🛠️ Tools & Skillset

 **Excel | DAX | Data Analysis | Data Visualization | Data Storytelling | Dashboard Design | KPI Development | Interactive Filters **

 ### 📚 What I Learned

 This project helped me improve my ability to **transform raw data into meaningful visual insights**. I learned how to design an effective dashboard layout, create KPIs and visualizations, use filters for interactive analysis, compare multiple dimensions of a dataset, and present complex information in a simple and understandable way.

 Most importantly, I learned that **good data visualization is not just about making charts—it’s about creating a clear flow that helps users understand the story behind the data.**

 #Excel #DataAnalytics #DataAnalyst #DataVisualization #DAX #BusinessIntelligence #Dashboard #DataStorytelling #AnalyticsProject #PortfolioProject
